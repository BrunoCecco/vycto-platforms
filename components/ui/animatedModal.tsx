    ...
    open && (
      ...
      >
        ...
        <motion.div
          ref={modalRef}
          className={cn(
            "relative z-50 flex max-h-[90%] min-h-[50%] max-w-[90%] flex-1 flex-col overflow-scroll rounded-lg border-2 bg-background md:max-w-[55%] md:rounded-2xl lg:max-w-[40%]",
            className,
          )}
          initial={{
            opacity: 0,
...
export const ModalContent = ({
  ...
  return (
    <div
      className={cn(
        "flex flex-1 flex-col bg-background p-8 md:p-10",
        className,
      )}
    >
      {children}
...
export const ModalFooter = ({
  ...
  return (
    <div className={cn("flex justify-end bg-background/50 p-4", className)}>
      {children}
    </div>
  );
};
...
const Overlay = ({ className }: { className?: string }) => {
  return (
    <motion.div
      ...
      className={`fixed inset-0 z-50 h-full w-full bg-background bg-opacity-50 ${className}`}
    ></motion.div>
  );
};
